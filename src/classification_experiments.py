...
def format_markdown_table(results: list[dict[str, Any]]) -> str:
    header = (
        "| Experiment | Ablation | Epochs | LR | Batch | Initial val acc | "
        "Final val acc | Final train acc |\n"
        "|---|---|---:|---:|---:|---:|---:|---:|"
    )
    rows = []
    for result in results:
        ...
        rows.append(
            "| {name} | {ablates} | {epochs} | {lr:g} | {batch} | {iva:.4f} | "
            "{fva:.4f} | {fta:.4f} |".format(
                name=result["name"],
...
def run_suite(dataset: str, profile: str, verbose: bool = False) -> list[dict[str, Any]]:
    return [
...
def main(argv: list[str] | None = None) -> None:
    ...
    for dataset in datasets:
        results.extend(run_suite(dataset, profile=args.profile, verbose=args.verbose))
